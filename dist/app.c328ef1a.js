...
function upload(selector) {
  ...
  var files = [];
  var onUpload = (_options$onUpload = options.onUpload) !== null && _options$onUpload !== void 0 ? _options$onUpload : noop; // пустая функция - чтобы не было ошибки если не указана функция

  var input = document.querySelector(selector);
  var preview = element('div', ['preview']);
  var openButton = element('btn', ['btn'], 'Открыть');
  var upload = element('btn', ['btn', 'primary'], 'Загрузить');
  ...
  var changeHandler = function changeHandler(event) {
    ...
    files.forEach(function (file) {
      ...
      reader.onload = function (event) {
        var src = event.target.result; // возвращает закодированное в формате base64 изображение
